...
import { DollarSign, TrendingUp, Heart, Search, Filter, Download, PieChart } from 'lucide-react'
...
export default function DonsPage() {
  ...
  const fetchDonations = async () => {
    try {
      ...
      const projectTotals = projectDonations.reduce((acc: Record<string, { amount: number; count: number }>, d: Donation) => {
        const name = d.projectName || 'Non specifie'
        if (!acc[name]) {
      ...
      const projectStatsArray: ProjectStat[] = Object.entries(projectTotals).map(([name, projectStat]) => ({
        name,
        amount: (projectStat as { amount: number; count: number }).amount,
        count: (projectStat as { amount: number; count: number }).count,
        percentage: projectAmount > 0 ? ((projectStat as { amount: number; count: number }).amount / projectAmount) * 100 : 0,
      }))
  ...
  const exportToCSV = () => {
    const headers = ['Date', 'Donateur', 'Email', 'Telephone', 'Type', 'Projet', 'Montant', 'Message']
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Gestion des Dons</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Suivi et statistiques des dons</p>
        </div>
        <button
          onClick={exportToCSV}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-xl shadow-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-green-100 text-sm font-medium">Montant total collecte</p>
              <p className="text-4xl font-bold mt-2">{stats.totalAmount.toLocaleString('fr-FR')} CHF</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-4xl font-bold mt-2">
                {stats.total > 0 ? (stats.totalAmount / stats.total).toFixed(2) : '0'} CHF
              </p>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-xl font-bold">Repartition par Type de Don</h2>
        </div>
              ...
              {typeStats.map((stat) => (
                <tr key={stat.type} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="py-4 px-4 text-right font-semibold text-green-600 dark:text-green-400">
                    {stat.amount.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CHF
                  </td>
            ...
            <tfoot className="border-t-2 border-gray-200 dark:border-gray-700 font-bold">
              <tr>
                ...
                <td className="py-3 px-4 text-right text-lg text-green-600 dark:text-green-400">
                  {stats.totalAmount.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CHF
                </td>
      ...
      {projectStats.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-xl font-bold">Repartition par Projet</h2>
          </div>
            ...
            <table className="min-w-full">
              ...
              <tfoot className="border-t-2 border-gray-200 dark:border-gray-700 font-bold">
                <tr>
                  ...
                  <td className="py-3 px-4 text-right text-lg text-green-600 dark:text-green-400">
                    {stats.project.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CHF
                  </td>
                  <td className="py-3 px-4 text-right">100%</td>
                  <td className="py-3 px-4"></td>
                </tr>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="MEMBERSHIP">Adhesion</option>
            </select>
          </div>
        </div>
      </div>
